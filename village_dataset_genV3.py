...
- sub-district randomisation (or real sub-district from recognized list)
- recoverable name typos (names only)
- richer synthetic 3-D
- optional component-level code-switching
"""
...
def parse_blocks(raw_text):
    ...
    ignore_phrases = [
        ...
        "Village Name", "村名", "District", "地區"
        # --- NEW HEADERS TO IGNORE ---
        "VILLAGES IN", "NON-INDIGENOUS"
    ]
...
def apply_localized_district(
    ...
    if rng.random() < 0.90:
        ...
        # re-sort later by canonical_order
...
def importance_weighted_omission(
    ...
    for chunk in chunks:
        ...
        if "building_number" in omitted_set:
            new_atoms = [a for a in chunk.atoms if a.label != "BUILDING_NUMBER"]
            if new_atoms:
...
def render_chunks(chunks: Sequence[Chunk], separator: str) -> str:
    parts: list[str] = []
...
class VillageAugmenter:
    ...
    def _apply_casing_simple(self, text: str, lang: str) -> str:
        """Kept for the initial name rendering; full case noise is applied later."""
    ...
    def build_chunks_from_pair(
        ...
        """Build the initial ordered chunks + the full target dict."""
        ...
        # Prefer real sub-district from recognized-villages list when present
        raw_sub = self._convert(pair.get(f"{prefix}_sub_district", "") or "", language)

        # Village suffixing
        if language == "en":
            if not raw_village.upper().endswith(("VILLAGE", "TSUEN")):
                if self.rng.random() < 0.40:
                    raw_village = f"{raw_village} Village"
                elif self.rng.random() < 0.20:
                    raw_village = f"{raw_village} Tsuen"
        else:
            if not raw_village.endswith(("村", "鄉", "围", "圍")):
                if self.rng.random() < 0.60:
                    raw_village = f"{raw_village}村"

        village = self._apply_casing_simple(raw_village, language)
        district = self._apply_casing_simple(raw_district, language)

        # Region inference
        if language == "en":
            dist_u = district.upper().replace(" DISTRICT", "") if district else ""
            if dist_u in {"SOUTHERN", "WAN CHAI", "EASTERN", "CENTRAL & WESTERN"}:
                raw_region = self.rng.choice(["HK", "H.K.", "Hong Kong", "HK Island"])
            elif dist_u in {"KWUN TONG", "WONG TAI SIN", "KOWLOON CITY", "SHAM SHUI PO", "YAU TSIM MONG"}:
                raw_region = self.rng.choice(["KLN", "Kln", "Kln.", "Kowloon"])
            else:
                raw_region = self.rng.choices(
                    ["New Territories", "N.T.", "NT", ""], weights=[25, 25, 25, 25]
                )[0] if self.rng.random() < 0.90 else self.rng.choice(["HK", "KLN"])
            region = self._apply_casing_simple(raw_region, language)
            if district and self.rng.random() < 0.20 and "DISTRICT" not in district.upper():
                district = f"{district} District"
        else:
            dist_c = district.replace("區", "").replace("区", "") if district else ""
            if dist_c in {"南", "灣仔", "湾仔", "東", "东", "中西"}:
                raw_region = self.rng.choice(["香港", "香港島", "港島"] if language == "zh-Hant" else ["香港", "香港岛", "港岛"])
            elif dist_c in {"觀塘", "观塘", "黃大仙", "黄大仙", "九龍城", "九龙城", "深水埗", "油尖旺"}:
                raw_region = self.rng.choice(["九龍", "九龙"] if language == "zh-Hans" else ["九龍"])
            else:
                opts = (["新界", "新界區"] if language == "zh-Hant" else ["新界", "新界区"]) + [""]
                raw_region = self.rng.choices(opts, weights=[60, 30, 10])[0] if self.rng.random() < 0.90 else self.rng.choice(["九龍", "香港"])
            region = raw_region
            suf = "區" if language == "zh-Hant" else "区"
            if district and self.rng.random() < 0.30 and not district.endswith(suf):
                district = f"{district}{suf}"

        # Building / house / DD Lot
        ...
        sub_dist = self._apply_casing_simple(raw_sub, language) if raw_sub else ""
        ...
        for kind, text, label in [
            ...
            ("building_number", b_number, "BUILDING_NUMBER"),
            ("phase", phase, "PHASE"),
            ("block", block, "BLOCK"),
            ("floor", floor, "FLOOR"),
            ("unit", unit, "UNIT"),
    ...
    def build_variant(self, pair: dict[str, str], group_id: str, variant_index: int) -> dict[str, Any]:
        language = self.rng.choices(
            ["en", "zh-Hant", "zh-Hans"], weights=[1, 1, 1]
        )[0]

        # Synthetic 3-D (always for villages; high rate)
        three_d = None
        if self.rng.random() < 0.85:
            three_d = synthetic_three_d(group_id, language, self.seed, (variant_index,))

        chunks, target = self.build_chunks_from_pair(
        ...
        # --- Sub-district localisation ---
        # Only randomise a sub-district when the pair did not already supply one
        # (recognized-villages list carries real sub-districts such as LAMMA NORTH).
        already_has_sub = any(c.kind == "sub_district" for c in chunks)
        if not already_has_sub:
            local_sc = apply_localized_district(chunks, language, rng)
            if local_sc:
                scenarios.append(local_sc)
                for c in chunks:
                    if c.kind == "sub_district" and c.atoms:
                        target["sub_district"] = c.atoms[0].text
                        break
        chunks = canonical_order(chunks, language)
        ...
        if variant_index > 0:
            ...
            if suf_sc:
                ...
                for c in chunks:
                    if c.kind == "district" and c.atoms:
                        target["district"] = c.atoms[0].text

        # --- Chinese messy separation ---
        if variant_index > 0 and rng.random() < 0.15 and language.startswith("zh"):
            for chunk in chunks:
        ...
        # --- Chinese region variation ---
        if variant_index > 0 and rng.random() < 0.60:
        ...
        # Render
        text = render_chunks(chunks, separator).strip()

        # Optional light code-switch (replace a few chunks with the other language)
        # For simplicity we keep the monolingual path dominant; a full parallel
        # mix would require a second language pass.  A cheap version:
        if (
            variant_index > 0
            ...
            # Switch region / district to English abbreviations when possible
            for chunk in chunks:
                if chunk.kind == "region" and chunk.atoms:
                    eng = ENGLISH_REGION_ABBREVIATIONS.get(
                        ENGLISH_REGION_CANONICAL.get(chunk.atoms[0].text.upper(), ""),
                        None,
                    )
                    if eng:
                        chunk.atoms[0].text = rng.choice(eng)
        ...
        output_format = {
            "line1": {
                ...
                "phase": target["phase"],
                "estate_name": target["estate_name"],
            },
            "line2": {
                "building_number": target["building_number"],
                "street_name": target["street_name"],
                "village_name": target["village_name"],
        ...
        return {
            ...
            "is_messy": is_messy or bool(omitted) or corruption is not None,
...
def _dedup_pairs(pairs: list[dict[str, str]]) -> list[dict[str, str]]:
    """Deduplicate by normalised English village name; prefer entries that carry sub-district."""
    seen: dict[str, dict[str, str]] = {}
    for p in pairs:
        key = (p.get("eng_village") or "").strip().upper()
        if not key:
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Village address synthesis V3 – dual gazetteer + full V2 augmentation suite"
    )
    parser.add_argument("--output", type=Path, default=Path("../processed_data/auto_train_villageV3.jsonl"))
    ...
    parser.add_argument("--mixed-rate", type=float, default=0.05,
                        help="Cheap code-switch probability")
    parser.add_argument("--seed", type=int, default=2026)
    args = parser.parse_args()

    # honour legacy flag
    if args.drop_rate != 0.15:
        args.component_drop_rate = args.drop_rate

    print("Parsing Place Name Gazetteer (2026)…")
    gaz_pairs = extract_bilingual_pairs(raw_text)
    print(f"  → {len(gaz_pairs)} pairs from gazetteer")

    print("Parsing Recognized Villages list (Lands Dept / Small House Policy)…")
    recog_pairs = extract_recognized_villages(RECOGNIZED_VILLAGES_RAW)
    ...
    if CC_CONVERTER is None:
        print(
            "⚠️  opencc not installed – Simplified Chinese conversion will be limited. "
    ...
    with args.output.open("w", encoding="utf-8") as f:
        for i, pair in enumerate(base_pairs):
            ...
            for v in range(args.variants_per_village):
                record = augmenter.build_variant(pair, group_id, v)
                f.write(json.dumps(record, ensure_ascii=False) + "\n")
                total += 1

    print(
        f"\nSuccess! Wrote {total} augmented village addresses "
        f"(en / zh-Hant / zh-Hans + full V2 augmentation suite) → {args.output}"
    )

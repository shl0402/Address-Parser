...
STANDARD_3D_WEIGHTS: tuple[tuple[str, int], ...] = (
    ...
    ("standard_room", 10),
    ("number_only_unit", 3),  # <--- NEW: e.g., just "582"
    ("hao_shi_unit", 10),        # <--- NEW: e.g., "12號室"
    ("alphabetic_unit", 8),
    ("leading_zero_unit", 5),
...
def block_text(components: Mapping[str, Any], language: str) -> str:
    ...
    # --- NEW FIX: Normalize ALS English abbreviations to formal terms ---
    if language == "en":
...
def apply_localized_district(chunks: list[Chunk], effective_language: str, rng: random.Random) -> str | None:
    ...
    # Pure random assignment: append it as a completely new chunk
    if rng.random() < 0.90:
        ...
        return "localized_district_randomized"
...
def apply_district_suffix_noise(chunks: Sequence[Chunk], fallback_language: str, rng: random.Random) -> str | None:
    ...
    for chunk in chunks:
        ...
        for atom in chunk.atoms:
            ...
            if is_english:
                # Target " District" or " DISTRICT" using regex
                if re.search(r'(?i)\s+DISTRICT$', text):
                    ...
                    if rng.random() < 0.20:
                        # Keep it capitalized if the source is all caps
                        suffix = " DISTRICT" if text.isupper() else " District"
                        atom.text = text + suffix
                        scenario_added = "added_district_suffix"
            else:
                # Target Traditional "區" or Simplified "区"
                if text.endswith("區") or text.endswith("区"):
                    if rng.random() < 0.80:
                        atom.text = text[:-1]
                        scenario_added = "removed_district_suffix"
                else:
...
def importance_weighted_omission(
    ...
    for chunk in chunks:
        ...
        if "building_number" in omitted_set:
            new_atoms = [atom for atom in chunk.atoms if atom.label != "BUILDING_NUMBER"]
            if new_atoms:
...
def sanitize_component_payload(
    ...
    """Wipe keys from component_payload if their entity was dropped from the input text."""
    sanitized = dict(payload)

    # Map entity label -> payload key(s)
    label_to_keys = {
        "REGION": ["region"],
        "DISTRICT": ["district"],
        "SUB_DISTRICT": ["sub_district"],
        "STREET_NAME": ["street_name"],
        "BUILDING_NUMBER": ["street_no_from", "street_no_to", "village_no_from", "village_no_to"],
        "VILLAGE_NAME": ["village_name"],
        "ESTATE_NAME": ["estate_name"],
        "PHASE": ["phase_name", "phase_no"],
        "BLOCK": ["block_no", "block_descriptor"],
        "BUILDING_NAME": ["building_name"],
        "FLOOR": ["floor_num", "floor_description"],
        "UNIT": ["unit_descriptor", "unit_no", "unit_portion"],
    }
    ...
    for label, keys in label_to_keys.items():
        if label not in observed_labels:
            for key in keys:
                if key in sanitized:
                    sanitized[key] = ""  # Sets omitted fields to an empty string to maintain schema
...
def build_training_example(
    *,
    ...
    fullwidth_punctuation_rate: float,  # <--- ADD THIS LINE
    english_upper_rate: float,   # <--- ADD THIS
    english_title_rate: float,   # <--- ADD THIS
    english_lower_rate: float,   # <--- ADD THIS
    english_mixed_rate: float,   # <--- ADD THIS
    seed: int,
    ...
    # --- NEW: Add messy spaces and wrong separators for Chinese addresses ---
    if variant_index > 0 and rng.random() < 0.15:  # 45% chance to apply this noise
        for chunk in chunks:
    ...
    # # --- NEW: Randomly drop specific "useless" information ---
    # # Only applies to augmented variants, 85% of the time
    # if variant_index > 0 and rng.random() < 0.85:
    #     for chunk in chunks:
    #         # Rebuild the atoms list, excluding the target label
    #         chunk.atoms = [
    #             atom for atom in chunk.atoms
    #             if atom.label != "BUILDING_NUMBER"
    #         ]
    #     scenarios.append("missing_building_number")
    # # ---------------------------------------------------------

    # --- NEW: Localized Sub-Districts (e.g., Wan Chai, Causeway Bay) ---
    ...
    local_scenario = apply_localized_district(chunks, effective_language, local_district_rng)
    if local_scenario:
    ...
    if variant_index > 0:
        ...
        suffix_scenario = apply_district_suffix_noise(chunks, effective_language, district_suffix_rng)
        if suffix_scenario:
    ...
    # --- NEW: Configurable English Case Randomization ---
    if variant_index > 0:
    ...
    # --- NEW: Apply Chinese Region Variation ---
    # (Using a 60% chance for variation when noise is eligible)
    if abbreviation_eligible and abbreviation_rng.random() < 0.60:
        chinese_scenario = apply_chinese_region_variation(chunks, effective_language, abbreviation_rng)
        if chinese_scenario:
    ...
    # --- NEW: Apply fullwidth translation to both English and Chinese ---
    ...
    base_chunk_languages = {
        chunk.source_language for chunk in base_chunks if chunk.source_language
    }
    current_chunk_languages = {
        chunk.source_language for chunk in chunks if chunk.source_language
    }
    if len(base_chunk_languages) >= 2 and len(current_chunk_languages) < 2:
        # Optional-context deletion must not accidentally turn a code-switched
        # row back into a monolingual row.
        chunks = copy.deepcopy(base_chunks)
        scenarios = [
            scenario
            ...
            not in {
                ...
                "english_case_variation",
            }
        ]
        scenarios.append("code_switch_preserved_after_noise")
        is_structurally_messy = any(
            scenario in {"mixed_punctuation", "irregular_whitespace"}
            for scenario in scenarios
        )

    # --- NEW: Compact / inverted floor+unit fusion (27LD, LD27, 27樓LD, ...) ---
    ...
    # Higher chance on augmented variants; still appear on the canonical variant
    fusion_rate = 0.28 if variant_index > 0 else 0.10
    ...
    # --- NEW: Sync component payload to remove dropped fields ---
    observed_labels = set(observed_components.keys())
    active_component_payload = sanitize_component_payload(
        component_payload(components, three_d),
        observed_labels
    )
    # ------------------------------------------------------------
    ...
    return {
        ...
        "components": active_component_payload,  # <--- UPDATED HERE
        "parallel_components": (
            {
...
input; an omitted component has no entity and no observed output.
`components` remains the complete canonical source reference for evaluation and
retrieval analysis, not a target that the tagger should hallucinate. Keep
examples with the same `group_id` in one split; the generator already enforces
this.
...
def build_dataset(args: argparse.Namespace) -> Path:
    ...
    if not (0.0 <= args.messy_rate <= 1.0):
        raise DatasetBuildError("--messy-rate must be between 0 and 1")
    if not (0.0 <= args.typo_rate <= 1.0):
        raise DatasetBuildError("--typo-rate must be between 0 and 1")
    if not (0.0 <= args.component_drop_rate <= 1.0):
        raise DatasetBuildError("--component-drop-rate must be between 0 and 1")
    if not (0.0 <= args.region_abbreviation_rate <= 1.0):
        raise DatasetBuildError("--region-abbreviation-rate must be between 0 and 1")
    if not (0.0 <= args.synthetic_3d_rate <= 1.0):
        raise DatasetBuildError("--synthetic-3d-rate must be between 0 and 1")
    if args.variants_per_address < 1:
        raise DatasetBuildError("--variants-per-address must be at least 1")
    if not (0.0 <= args.mixed_variants_per_address <= 1.0):
        raise DatasetBuildError("--mixed-variants-per-address must be between 0.0 and 1.0")
    ...
    try:
        for feature, source_ref, feature_index in iter_all_features(
            ...
            if not usable_languages:
                ...
                continue
            # --- NEW: Randomly skip source rows missing specific elements ---
            ref_components = components_by_language[usable_languages[0]]
            filter_rng = stable_rng(args.seed, source_ref.as_string(), feature_index, "missing_info_filter")
            ...
            for comp_key, skip_prob in filter_rates.items():
                # If the probability is set and the element doesn't exist in the source
                if skip_prob > 0.0 and not clean_text(ref_components.get(comp_key)):
            ...
            for language in usable_languages:
                ...
                for profile_index, (real_profile, source_kind) in enumerate(profile_options):
                    for variant_index in range(args.variants_per_address):
                        ...
                        target_synthetic_rate = 0.80 if is_village else 0.95

                        if (
                            profile is None
                            and stable_unit_interval(
                        args.seed, group_id, profile_index, variant_index, "synthetic_gate"
                        )
                                < target_synthetic_rate
                        ):
                        ...
                        example = build_training_example(
                            ...
                            component_drop_rate=args.component_drop_rate,  # <--- ADD THIS LINE
                            region_abbreviation_rate=args.region_abbreviation_rate,
                            fullwidth_punctuation_rate=args.fullwidth_punctuation_rate,
                            english_upper_rate=args.english_upper_rate,  # <--- ADD
                            english_title_rate=args.english_title_rate,  # <--- ADD
                            english_lower_rate=args.english_lower_rate,  # <--- ADD
                            english_mixed_rate=args.english_mixed_rate,  # <--- ADD
                            seed=args.seed,
                        )
                    if not example["text"]:
                ...
                for chinese_language in (
                    ...
                    for (
                        ...
                        if stable_unit_interval(args.seed, group_id, profile_index,
                            ...
                            three_d_source = source_kind
                            is_village = bool(clean_text(english_components.get("village_name")))
                            target_synthetic_rate = 0.80 if is_village else 0.95
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.9,
        help="For 2D records, chance that a variant receives marked synthetic floor/unit syntax",
    )
    parser.add_argument(
    ...
    parser.add_argument("--english-upper-rate", type=float, default=0.15, help="Probability of all-caps English text")
    parser.add_argument("--english-title-rate", type=float, default=0.25, help="Probability of Title Case English text")
    parser.add_argument("--english-lower-rate", type=float, default=0.55, help="Probability of lowercase English text")
    parser.add_argument("--english-mixed-rate", type=float, default=0.05, help="Probability of mixed case English text")
    parser.add_argument(
        "--simplified",
